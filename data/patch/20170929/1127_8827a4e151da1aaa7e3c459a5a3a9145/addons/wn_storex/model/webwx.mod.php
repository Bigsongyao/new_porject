...
function webwx_cookie($baseinfo)
{
	...
	if ($status == '0') {
		$post->BaseRequest = array('Uin' => $baseinfo['wxuin'], 'Sid' => $baseinfo['wxsid'], 'Skey' => $baseinfo['skey'], 'DeviceID' => 'e' . rand(10000000, 99999999) . rand(1000000, 9999999));
...
function webwxstatusnotify($request)
{
	...
	$params = array('BaseRequest' => $request['request'], 'Code' => 3, 'FromUserName' => $request['fromusername'], 'ToUserName' => $request['fromusername'], 'ClientMsgId' => time());
	$result = webwx_post($url, $params);
...
function webwx_sync($request, $synckey)
{
	...
	$params = array('BaseRequest' => $request['request'], 'SyncKey' => $synckey, 'rr' => time());
...
function synccheck($request, $synckey_info, $cookie)
{
	...
	$params = array('r' => time() * 1000, 'skey' => $request['request']['Skey'], 'sid' => $request['request']['Sid'], 'uin' => $request['request']['Uin'], 'deviceid' => $request['request']['DeviceID'], 'synckey' => $synckey, '_' => time() * 1000);
...
function webwx_getcontact($post, $post_url_header)
{
	...
	$params = array('BaseRequest' => array('Uin' => $post['uin'], 'Sid' => $post['sid'], 'Skey' => $post['skey'], 'DeviceID' => $post['deviceid']));
...
function webwx_sendmsg($params, $word, $fromusername, $tousername = 'filehelper')
{
	...
	$data = array('BaseRequest' => $params['request'], 'Msg' => array('Type' => 1, 'Content' => urlencode($word), 'FromUserName' => $fromusername, 'ToUserName' => $tousername, 'LocalID' => $clientMsgId, 'ClientMsgId' => $clientMsgId));
...
function webwx_sendimg($params, $image, $fromusername, $tousername = 'filehelper')
{
	...
	$data = array('BaseRequest' => $params['request'], 'Msg' => array('Type' => 3, 'MediaId' => $uploadimg['MediaId'], 'FromUserName' => $fromusername, 'ToUserName' => $tousername, 'LocalID' => time() * 1000 . substr(uniqid(), 0, 5), 'ClientMsgId' => time() * 1000 . substr(uniqid(), 0, 5)), 'Scene' => 0);
	$result = curlPost($url, $data);
	$result = @json_decode($result, true);
	return $result['BaseResponse']['Ret'] == 0;
}
function uploadimg($params, $fromusername, $tousername, $image)
{
	...
	$uploadmediarequest = json_encode(array('BaseRequest' => $params['request'], 'ClientMediaId' => time() * 1000 . mt_rand(10000, 99999), 'TotalLen' => $file_size, 'StartPos' => 0, 'DataLen' => $file_size, 'MediaType' => 4, 'UploadType' => 2, 'FromUserName' => $fromusername, 'ToUserName' => $tousername, 'FileMd5' => md5_file($image)));
	$multipart_encoder = array('id' => 'WU_FILE_5', 'name' => $image, 'type' => 'images/jpeg', 'lastModifieDate' => gmdate('D M d Y H:i:s TO', $file_time) . ' (CST)', 'size' => $file_size, 'mediatype' => 'pic', 'uploadmediarequest' => $uploadmediarequest, 'webwx_data_ticket' => $webwx_data_ticket, 'pass_ticket' => $params['pass_ticket'], 'filename' => '@' . $image);
...
function webwx_post($url, $param, $jsonfmt = true, $post_file = false)
{
	...
	$header = array('User-Agent: Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/48.0.2564.109 Safari/537.36');
	if ($jsonfmt) {
...
function webwx_get($url, $api = false, $cookie = '')
{
	...
	$header = array('User-Agent: Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/48.0.2564.109 Safari/537.36', 'Referer: https://wx.qq.com/', 'Cookie: ' . $cookie);
