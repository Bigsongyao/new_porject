...
function cloud_m_prepare($name) {
	...
	$pars['module'] = $name;
	$dat = cloud_request('http://cloud.xiaoyinnet.com/gateway.php', $pars);
	if (is_error($dat)) {
		return $dat;
	}
	return true;
...
function cloud_sms_send($mobile, $content, $postdata = array()) {
	...
	$response = cloud_request('http://cloud.xiaoyinnet.com/gateway.php', $pars);
...
function cloud_download($path, $type = '') {
	...
	$dat = cloud_request('http://cloud.xiaoyinnet.com/gateway.php', $pars, $headers, 300);
	if(is_error($dat)) {
...
function cloud_cron_create($cron) {
	...
	$result = cloud_request('http://cloud.xiaoyinnet.com/gateway.php', $pars);
...
function cloud_auth_ip(){
	...
	$pars['cron_id'] = $cron_id;
	$pars['status'] = $status;
	$result = cloud_request('http://cloud.xiaoyinnet.com/gateway.php/a=ipcl', $pars);
	...
	$auth_url = 'http://cloud.xiaoyinnet.com/gateway.php/a=ipcl&__auth=' . $query;
...
function cloud_auth_url($forward, $data = array()){
	...
	$query = base64_encode(json_encode($auth));
	$auth_url = 'http://cloud.xiaoyinnet.com/gateway.php/a=passwort&__auth=' . $query;
...
function cloud_flow_master_post($flow_master) {
	...
	$dat = cloud_request('http://cloud.xiaoyinnet.com/gateway.php', $pars, array(), 300);
	if(is_error($dat)) {
		return error(-1, '网络存在错误， 请稍后重试。' . $dat['message']);
	}
